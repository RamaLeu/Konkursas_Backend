...
const Apartaments = () => {
    ...
    let removeApartament = (e) =>{
        remove(ref(db, 'apartaments/' + e.target.id));
        console.log(ref(db, 'users/' + e.target.id));
    }
    readData()
    return (
        <div>
                       ...
                       <th>
                        ...
                        {apartData.map((item, index) =>{
                            return <tr key={index}>
                                    ...
                                    {index !==0 ? <td><a href="../apartaments" className="button" id={index} onClick={(e) => removeApartament(e)}>X</a></td> : <td></td>}
                                    </tr>
                        })}
               
               </tbody>
            </table> 
            <Link to="/createApartament">Prideti buta</Link>
        </div>
    )
}
